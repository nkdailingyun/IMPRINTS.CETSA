...
## Introduction  
This package is primarily designed to process IMPRINTS formats of MS-CETSA result, which was first introduced in Dai et al. 2018 Cell paper ("Modulation of Protein-Interaction States through the Cell Cycle").  
...
# to only keep the proteins from the right species
Chem_c <- Chem[grep("Homo sapiens", Chem$description), ]
```

5. Remove proteins without quantitative information by default using `ms_clean()`.  
By default the Keratin family proteins, Bovine serum proteins, and Human Trypsin enzymes are also removed.  
