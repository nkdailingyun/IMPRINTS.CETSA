...
#' @param averagecount Filter by the median across temperatures of the associated abundance
#'   count of proteins or filter by each
...
imprints_rearrange <- function (data, nread = 9, repthreshold = 0.75, withabdreading = TRUE,
  ...
  if (withabdreading){
    ...
    message(paste(length(unique(refdata$id)), "proteins are measured at",
                   basetemp, "and they are kept."))
    data <- subset(data, id %in% refdata$id)
  }

  if (length(extraid)) {
    message(paste("To include extra", length(extraid), "proteins as specified."))
    extraid <- setdiff(extraid, unique(data$id))
    data <- rbind(data, data_copy[which(data_copy$id %in% extraid),])
  }

  d1 <- tidyr::gather(data[, c(1, 3, 4:(3 + nread))], treatment,
                      reading, -id, -condition)
  d1 <- tidyr::unite(d1, combinedcol, condition, treatment,
                     sep = "_")
  ...
  if (length(pos) > 1) {
    ...
    if (averagecount & countthreshold > 0) { # filter on mmedian across temperatures
  ...
  ms_filewrite(data1, paste0(dataname, "_data_pre_normalization.txt"), outdir = outdir)
