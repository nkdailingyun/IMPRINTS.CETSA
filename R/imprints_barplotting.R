...
imprints_barplotting <- function(data, treatmentlevel=NULL, setlevel=NULL, corrtable=NULL, plotseq=NULL,
  ...
  plots <- plyr::dlply(cdata, plyr::.(id), .fun=ms_innerbarplotting,
                       withset=withset, colorpanel=colorpanel)
  if (ret_plot){
    return(plots)
  }
  ...
  if (external){
    external_graphs(F) # switch off the external graphs
  }
